...
- **Mutation class:** `docs_only` | `product_settings` | `catalogue_product` | `fs_mutation` | `escalate_break_loop` | hybrid Continuity X **D** / **M** / **D+M**
...
- **Clearance #4 status (if Continuity X / TNA):** n/a | DRAFT written / approval pending | approved-for-named-map (exact map; nest execute held) | nest-execute done (A+E / path_batch) | A+E reserved (not declined)
...
- **Next FAW lock hint:** (e.g. WorkSpace only + Continuity X; dedicated hermes A+E/execute candidate or other justified | WorkSpace only (TNA) + continue Continuity X clearance / keep-at-root | n/a — never dashboard default; never Complete while WorkSpace remains)
...
- [ ] 3. planner → 03-plan/
- [ ] 4. User plan gate → n/a (`docs_only` | `product_settings`) | pending | approved (`fs_mutation` | `catalogue_product` + Continuity Q6) | Continuity X D/M/D+M
