...
- **Mutation class:** `docs_only` | `product_settings` | `fs_mutation` | `escalate_break_loop`
- **Batch approval:** `n/a` | `pending` | `approved` (date/note) | Continuity gate `X`/`P`/`N`/`H` (escalate)
- **Pending user gates:** (e.g. taxonomy final sign-off | must-preserve review | Continuity X/P/N | none | waived: …)
...
- **STAGE:** (e.g. prompt-betterment | research → plan → same-run implement if docs_only/product_settings ready | escalate Continuity gate → close without implementer)
- **Next FAW lock hint:** (e.g. WorkSpace only + Continuity X XL/git-strategy | n/a)
