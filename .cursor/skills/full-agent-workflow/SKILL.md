...
- **Mandatory** when the plan is **`catalogue_product`** and Continuity Q6=A (or Continuity requires a plan gate): org-repo catalogue **code+data** changes — STAGE 1 hold → STAGE 2 after yes (+ load-strategy amendments such as **L-both**). Not corpus `fs_mutation`; still not same-run auto-continue while the gate is pending.
...
- **`catalogue_product`:** org-repo catalogue code+data (e.g. dashboard generator/HTML/data siblings). Zero WorkSpace/corpus moves. Prefer **L-both** when Continuity/gate picks it: JSON + classic JS module; `file://` module vs HTTP `fetch`.
- **STAGE 1 / same-run auto-continue:** After planner, if **`docs_only` or `product_settings`** + **`ready_to_implement: yes`** + no plan gate → continue implement → mid git (as appropriate) → audit → self-improver → **final closing-pass git** → Close **in the same run** (do not wait for a separate user “go” unless they asked to review). If `fs_mutation` (incl. remote-config), **`catalogue_product`** (pending Continuity Q6 gate), **`escalate_break_loop`**, or not ready → stop at plan / Continuity gate / blocking questions.
- **Workstream rotation (post–Cycle 22):** Do **not** default Next FAW to Organisation dashboard polish. Items 20–22 are done for now. Researcher must **justify** workstream vs ROADMAP each cycle; default Next FAW = WorkSpace Continuity X / `#4` (after **approved-for-named-map**, prefer dedicated hermes/named-map **A+E**/execute when Continuity binds) or research-justified alternative — never Complete while WorkSpace remains.
