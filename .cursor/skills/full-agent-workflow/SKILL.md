...
- **`product_settings` (no mandatory corpus gate):** editor/IDE extension install-switch, settings keys, and/or local evidence fixtures outside the catalogue — **not** corpus `fs_mutation`. Plan-gate **n/a**; proceed when `ready_to_implement: yes` (same-run). Soft **Reload window** / Marketplace UI only if CLI cannot install and no plan fallback — tip the user once; do not treat Reload as Critical or invent recurring user chores.
