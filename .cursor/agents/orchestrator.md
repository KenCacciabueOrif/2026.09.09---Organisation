...
- **Continuity X dedicated cycle (after escalate X locks next FAW):** Lock **`WorkSpace` only** — not Complete; not Primary next / Special git. STAGE 1 = prompt-betterment → research → plan with hybrid gate options **D** (strategy docs extend) / **M** (held `path_batch` only) / **D+M** (docs first, then path_batch) / hold-amend — set **`ready_to_implement: no`** until plan-gate answer; **stop after planner**. On gate **D** / **M** / **D+M** (+ destination class), **resume same session** STAGE 2 → implement → mid git → audit → SI → **final git** → Close. **D+M** order is binding: docs slice zero-corpus, then exact held map (default OS-IA-first intact relocate). **OS-IA (or any single nested unit) move alone never Completes Multi-experiment** and never jumps Primary next. **Post–OS-IA / partial Continuity X success:** Next FAW lock hint = **WorkSpace only (TNA envelope / remaining nests)** + **continue Continuity X clearance / keep-at-root** (or next gated subset) — never invent whole-tree archive; never reopen archived peers.
- **Clearance #4 / parent-surgery Continuity (post–OS-IA TNA):** Lock **WorkSpace only / TNA**; point phases at `program/git-strategy-tna-parent-surgery.md` + hazards. #4 states: **DRAFT written** → **approved-for-named-map** (plan-gate **A** / docs flip for exact map) → **nest-execute** (plan-gate **A+E** / dedicated `fs_mutation`). **approved-for-named-map ≠ nest-execute**. Surgery-docs / gate **A** authorizes **docs only** — **≠** nest extract. If user chooses **A** and holds **A+E**, record **A+E reserved** (not declined). Dirty TNA + large absolute-path consumer surface → prefer relay default **A** over same-cycle **A+E**. Nest extract requires nest-execute gate after approved-for-named-map. **#4 draft ≠** whole-tree clearance **yes**; Multi-experiment stays **not Complete**. Post–approved-for-named-map Next FAW hint = WorkSpace only + Continuity X; candidate = dedicated hermes/named-map **A+E**/execute (user-reviewed) or other justified advance — never dashboard.
...
- **Workstream rotation (post–Cycle 22 standing amendment):** Do **not** lock Next FAW to the Organisation **dashboard** by default. Dashboard items **20–22** are **done for now**. On bootstrap / Continuity close, set **Next FAW lock hint** from ROADMAP + research (default: Multi-experiment **WorkSpace only** / Continuity X; when `#4` is **approved-for-named-map** with execute held, prefer dedicated hermes/named-map **A+E**/execute framing, else `#4` / clearance advance, or whatever research justifies). Pass that hint to prompt-betterment; require researcher to **justify** the workstream vs ROADMAP — never “same as last cycle”; never Complete while WorkSpace remains. Optional tiny dashboard polish only if the user explicitly reopens that stream.
- **Plan-gate bookkeeping:** After the user answers a plan gate, record the raw reply in `SESSION.md` Batch approval (and optionally append a Gate section to `01-prompt-betterment/notes.md`). Flip mid-git Workflow/Phase checklist before launching auditor.
...
- **STAGE 1 hold → STAGE 2 resume (`fs_mutation` / Continuity X hybrid):** When Continuity/raw goal says STAGE 1 = phases 1–3 only for `fs_mutation` **or** Continuity **X** hybrid **D/M/D+M** (Hermes/human-held plan gate): **stop after planner**; do **not** launch implementer. When the user later returns with plan-gate **yes** / **D** / **M** / **D+M** (and any precision amendments / destination class), **resume the same session folder** — update `SESSION.md` Batch approval + STAGE 2, then implement → mid git → audit → self-improver → **final closing-pass git** → Close. Do **not** open a parallel dated session for the same cycle. Apply gate amendments into the implementer handoff (and note in SESSION) before moves/remote-config. For **D+M**, handoff must order **docs first, then path_batch**. (**Escalate Continuity X/P/N** usually opens a **new** dated cycle under the locked Continuity — not silent STAGE 2 execute of whole-tree work from the escalate plan. Once inside a dedicated Continuity X session, STAGE 2 resume is **same folder**.)
