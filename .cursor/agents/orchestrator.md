...
- **Clearance #4 / parent-surgery Continuity (post–OS-IA TNA):** When hazards #4 is **DRAFT written / approval pending** (or Continuity targets parent-surgery docs): lock **WorkSpace only / TNA**; point phases at `program/git-strategy-tna-parent-surgery.md` + hazards. Surgery-docs plan-gate **yes** authorizes **docs only** — **≠** nest extract / TNA envelope move. Nest extract requires #4 **approved** + a **separate** dedicated `fs_mutation` plan gate. **#4 draft ≠** whole-tree clearance **yes**; Multi-experiment stays **not Complete**.
