...
2. Draft `plan.md` with:
   ...
   - **Mutation class** (required): `docs_only` | `product_settings` | `fs_mutation`.
     ...
     - **`product_settings`:** editor/IDE extension install-switch, user `settings.json` keys, and/or **local evidence fixtures** outside the org-repo catalogue (e.g. regenerate a header on a prework file). **Not** corpus `fs_mutation` — **plan-gate n/a**; same-run continue when `ready_to_implement: yes` (like `docs_only`). Prefer this label over forcing `docs_only` when product/env mutation occurs. Name primary + **fallback** extension IDs when Marketplace vs Open VSX availability may differ. Soft user **Reload** / UI install = optional tip, not a Continuity/plan gate.
