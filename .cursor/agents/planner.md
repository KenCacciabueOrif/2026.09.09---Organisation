...
2. Draft `plan.md` with:
   ...
   - Explicit non-goals
   - Rollback / risk notes
   - **Mutation class** (required): `docs_only` | `fs_mutation` (moves/renames/deletes or other corpus path changes). If `fs_mutation`: name the batch; require **user approval before implementer**; treat each `.git` root as an **atomic unit**; secrets/deps move as opaque payload (never read secret contents); fail-closed on must-preserve / default-protect paths. Include a **“What the user is approving”** section: plain-language meaning of yes/no, the exact path map, and **pros / cons (tradeoffs)** (e.g. bookmarks break, parents created, partial-batch risk) so the orchestrator can relay an informed plan gate. If `docs_only`: state **zero intentional corpus FS mutations** and how implementer will attest.
