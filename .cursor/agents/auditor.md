...
- [ ] **`escalate_break_loop` / anti-loop success:** If plan mutation class is **`escalate_break_loop`** (or research `concrete_advance_candidate: none`): Continuity gate **X / P / N / H** recorded with raw reply; Continuity A theater **not** the cycle outcome; **`ready_to_implement: no`** → implementer skipped (attestation OK) + mid git skipped; zero corpus FS / zero remote-config; Multi-experiment **Remaining: WorkSpace only / not Complete**; if Continuity **X**, Next FAW hint = dedicated XL/whole-tree git-strategy + future plan gate (still WorkSpace only). Process **pass** = honest escalate, not a failed cycle.
- [ ] **Continuity X dedicated / hybrid D+M:** If Continuity **X** / hybrid gate: evidence of plan-gate **D** / **M** / **D+M** (or hold) before STAGE 2; for **D+M** docs-first then path_batch attested; strategy fate/clearance material (not Continuity A theater); single-unit move (e.g. OS-IA) leaves Multi-experiment **not** Complete / **not** Primary next; WorkSpace remainder (TNA / nests) still at root; Next FAW hint = WorkSpace only + continue Continuity X clearance / keep-at-root.
- [ ] **Clearance #4 / TNA parent-surgery:** If plan targeted #4 / surgery docs: `program/git-strategy-tna-parent-surgery.md` exists (or plan explains absence); hazards #4 is **DRAFT written / approval pending** **or** **approved-for-named-map** (exact map; nest still live when gate was **A**) **or** nest-execute with **A+E**/path_batch evidence — never inflate DRAFT→approved-for-named-map→nest-execute without the matching gate; **A+E reserved** (≠ declined) is valid when gate was **A** only; zero nest/TNA moves when mutation was `docs_only`; whole-tree clearance still **NO** unless plan+evidence flipped all criteria; Multi-experiment **not** Complete; Next FAW still WorkSpace + Continuity X (never dashboard Complete-jump); nest model ignored-clones (not submodule teardown invent).
