...
- [ ] **Clearance #4 / TNA parent-surgery:** If plan targeted #4 / surgery docs: `program/git-strategy-tna-parent-surgery.md` exists (or plan explains absence); hazards #4 is **DRAFT written / approval pending** **or** explicitly **approved** with separate nest-gate evidence — never inflate DRAFT→approved from docs gate alone; zero nest/TNA moves when mutation was `docs_only`; whole-tree clearance still **NO** unless plan+evidence flipped all criteria; Multi-experiment **not** Complete; nest model ignored-clones (not submodule teardown invent).
