...
- **Clearance #4 / TNA parent-surgery docs:** When the plan creates/extends `program/git-strategy-tna-parent-surgery.md` or hazards #4 **DRAFT written / approval pending**: **zero** nest / TNA envelope / WorkSpace moves this cycle. Do **not** claim #4 **approved** or `clearance_whole_tree` **yes**. Treat nests as **ignored nested clones** (not submodules) — no submodule teardown / no auto-commit of dirty TNA “to enable” docs. Nest extract only when a later plan is gated `fs_mutation` **after** #4 approved + dedicated path_batch gate.
