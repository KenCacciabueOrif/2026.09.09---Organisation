...
### Docs-only / product_settings / corpus FS safety
...
- When the plan is **`product_settings`**: execute extension/settings/fixture steps only. **Zero** corpus/catalogue moves. Attest no ROADMAP/WorkSpace path mutations. Prefer CLI install/uninstall; verify via `extensions.json` / extension folder / settings Read when Shell/`--list-extensions` blocked.
- When the plan is **`catalogue_product`**: change only org-repo `catalogue/` (and session logs) per plan/gate. **Zero** WorkSpace/corpus FS. Attest mutation-class honesty. If gate chose **L-both**: emit/verify JSON + classic JS module siblings; HTML boot prefers module on `file://`, `fetch` on HTTP; fatal UX on load failure — do not re-inline mega JSON.
- **Local installed-extension `dist/` / `src/` patch:** When the plan patches `~\.cursor\extensions\…` (e.g. 42-header frame/`extractHeader`): backup first; patch template **and** matching detect regex together; keep Continuity field widths (e.g. AUTHOR ≥41). Log **re-apply after Marketplace/Open VSX update** (upgrade overwrites local patch) + rollback. Prefer **generator patch** over `# noqa` / flake8 ignore / raising max-line-length when Continuity says so. **Never** stage extension product paths or sibling fixtures into the org-repo unless Continuity explicitly publishes them.
- **Cursor Open VSX vs Marketplace:** Cursor CLI often resolves the Open VSX–style catalog. If primary extension ID fails with **not found** / catalog 404 and research/plan named a **fallback** ID (often available on both Marketplace and Open VSX), **install the fallback**, log primary failure + delivered ID/version, and continue AC verify. Do **not** stall the cycle hoping Marketplace-only primary appears. Soft user Marketplace/VSIX tip only when **no** plan fallback remains — one-shot tip, not a recurring chore.
- **Soft Reload:** After extension install/switch **or after patching `dist/extension.js` (or equivalent)**, note **Reload Cursor window** as soft remaining (live insert/update-on-save). Do **not** block `complete` status on Reload; do **not** set `rework_owner: user` solely for Reload. Template-math / settings-equivalent header regenerate is an allowed verify method when Command Palette cannot be invoked from the agent.
...
- **WorkSpace / Appendix A:** Strategy docs (`program/git-strategy-workspace-hazards.md`) alone **never** authorize corpus moves. Execute scoped `_backups`/`_quarantine` isolation **only** when the plan is gated `fs_mutation` with Continuity opt-in + recorded plan-gate approval. **Never** invent whole-tree `WorkSpace` archive; **never** mark Multi-experiment Complete while `WorkSpace` remains (including after remote-config clear **or** after a single nested-unit archive such as OS-IA).
- **Continuity X hybrid D / M / D+M:** Honor plan-gate letter. **D** = strategy/honesty docs only + zero-corpus attestation. **M** / **D+M** path_batch = exact held map only (default OS-IA-first intact relocate). For **D+M**: finish docs slice **before** any move; log STAGE 2A then 2B. Immediately before move: live dirty porcelain recount (may differ from research) + `git worktree list` on **nested clone path** only. Remotes path-only; reverse-move note; **NO_AUTO_COMMIT** when Continuity/Q says so. Post-move honesty: INDEX/inventory/ROADMAP **Remaining: WorkSpace only** (TNA / remaining nests); **not** Complete; **not** Primary next. Next FAW hint = WorkSpace TNA envelope + continue Continuity X clearance / keep-at-root.
- **Clearance #4 / TNA parent-surgery docs:** When the plan creates/extends `program/git-strategy-tna-parent-surgery.md` or flips #4 to **approved-for-named-map** under plan-gate **A** / `docs_only`: **zero** nest / TNA envelope / WorkSpace moves this cycle. Record status as **approved-for-named-map** (not nest-execute). If plan notes **A+E reserved**, keep that language — **not** “declined.” Do **not** claim nest-execute or `clearance_whole_tree` **yes**. Treat nests as **ignored nested clones** (not submodules) — no submodule teardown / no auto-commit of dirty TNA “to enable” docs. Nest extract only when plan-gate **A+E** / dedicated `fs_mutation` path_batch authorizes the **exact** approved-for-named-map after consumer preflight.
...
- **Continuity A / docs_only after multi-remote clear:** When the plan requires inventory/INDEX honesty and research shows multi-remote **CLEARED**, replace stale **“live multi-remote uncleared”** (or equivalent) with **Cleared (cycle cite)** + still-at-root / **not** Complete. Strategy attestation may be **no material hazard delta** vs prior cycle — still write the attestation. Optional historical size-cell refresh is **not** hard AC unless the plan says so; do **not** flip `clearance_whole_tree` or mark the ROADMAP row Complete.
