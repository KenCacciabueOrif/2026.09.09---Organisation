...
2. **Question the user** — Ask focused clarifying questions (prefer a short batch of 5–10). Cover:
   ...
   - If the goal includes **git pull / sync from origin**: use the **pull-cycle question pack** in `.cursor/skills/full-agent-workflow/references/pull-cycle.md` (remote/branch / ff-only / dirty autonomy / **Q3c behind+allowlisted → stash→ff→pop** / git-root-only / agent Shell / fail-closed / session docs). Do not assume “user can pull” means agent can; on Windows note GfW vs MSYS for dirty gate, allowlist commit/stash, and pull. FAW defaults: abort only for unrelated dirty → `dirty_working_tree`; when behind+allowlisted dirty, disclosed Choose default is Q3c **A** (not commit-first).
