...
2. **Codebase pass** — Search the repo for relevant files, patterns, and prior art. Summarize in `codebase-findings.md` (paths + why they matter).
   ...
   - **Post–OS-IA / clearance #4 parent-surgery:** When OS-IA is archived and residual is TNA + ignored nests: read `program/git-strategy-workspace-hazards.md` **and** `program/git-strategy-tna-parent-surgery.md` if present. Classify nests as **ignored nested clones** (not submodules). Split #4: **DRAFT written** vs **approved-for-named-map** (exact map; docs only) vs **nest-execute** (dedicated path_batch / **A+E**). **approved-for-named-map ≠ nest-execute**. Prefer material #4 docs advance when DRAFT missing/thin; when already approved-for-named-map with execute held, prefer Next FAW candidate = dedicated hermes/named-map **A+E**/execute **or** other justified advance. Flag dirty TNA + large absolute-path consumer surface as reason to prefer gate **A** over same-cycle **A+E**. **#4 draft ≠** whole-tree clearance **yes**; Next FAW = WorkSpace only / TNA + Continuity X — never dashboard; never Complete while WorkSpace remains.
